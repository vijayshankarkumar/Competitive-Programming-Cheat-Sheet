...
- If first and last element of the sorted array is same then all the elements are same, this seems obvious but lets take a scenerio where you have to check if all the elements of a mutable array (i.e. it supports update or insert) has same frequence or not, then you can take a sorted set data structure such has **std::multiset<int>** which stores the frequency of all the elements and check whether first element is equal to the last element in the set or not. 
